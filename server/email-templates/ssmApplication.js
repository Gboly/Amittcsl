export const ssmApplicationTemplate = `<html>
  ...
  <body>
    <div class="container">
      <div class="card">
        <div class="header">
          <h1 class="title">New Stocks and Shares Management Application</h1>
          <p class="sub">Submitted on {{createdAt}}</p>
        </div>
        ...
        <div class="content">
          ...
          <table class="kvs">
            ...
            <tr><th>Name</th><td>{{name}} {{#if middleName}}{{middleName}} {{/if}}{{surname}}</td></tr>
          ...
          <table class="kvs">
            <tr><th>Dematerialization</th><td>{{#if serviceDematerialization}}✅ Yes{{else}}❌ No{{/if}}</td></tr>
            <tr><th>Recovery of Dividends</th><td>{{#if serviceDividends}}✅ Yes{{else}}❌ No{{/if}}</td></tr>
            <tr><th>Consolidation of Shares</th><td>{{#if serviceConsolidation}}✅ Yes{{else}}❌ No{{/if}}</td></tr>
            <tr><th>Creation of CSCS Account</th><td>{{#if serviceCSCS}}✅ Yes{{else}}❌ No{{/if}}</td></tr>
            {{#if otherService}}
              <tr><th>Other Service</th><td>{{otherService}}</td></tr>
            {{/if}}
          </table>
          ...
          <p>
            {{#if declarationConfirmed}}
              ✅ The applicant confirmed that all information provided is true and accurate.
            {{else}}
        ...
        <div class="foot">
          This application was generated automatically. Please follow up with the applicant via <span class="code">{{email}}</span>.
        </div>
      </div>
    </div>
  </body>
